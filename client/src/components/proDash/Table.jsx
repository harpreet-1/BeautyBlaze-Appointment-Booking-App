...
export default function Table() {
  ...
  function handleStatus(e, index) {
    fetch(
      `http://localhost:7700/appointment/status/${fetchedData[index]._id}`,
      {
        method: "PUT",
        headers: {
      ...
      .then((data) => {
        ...
        setFetchedData(
          [...fetchedData],
          (fetchedData[index].status = e.target.value)
        );
      })
            ...
            return (
              <React.Fragment key={index}>
                <tr key={el._id}>
                  ...
                  <td>
                    <div className={el.status} id="statusBox">
                      {el.status}
                    </div>
                  </td>
                  ...
                  <tr>
                    <td colSpan="3">
                      <div className="details-box">
                        <pre>
                          Email: {fetchedData[selectedRow].customerID.email}
                        </pre>
                        <pre>
                          Service:{" "}
                          {detail.service ? detail.service.name : "Facial"}
                        </pre>
                        <select
                          name="status"
                          id="status"
